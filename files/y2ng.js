let y2ng = {
  ...
  maxy: E(2),
  autobuy: [false, false, false, false, false, false, false, false]
}
...
function multiplyer(n) {
  ...
  let m1 = (n == 7 && yb.z.gte(2)) ? E(16).pow(ys.ba1) : E(1)
  let m2 = ([1, 2, 4, 8].includes(n)) ? yklng[0].p.div(ores.upg.copper[0] ? 1 : 8).add(1) : E(1)
  let m3 = yb.z.gte(12) ? E(2).pow(yb.z.div(2)) : E(1)
  return E(2).pow(y2ng.glevel[n - 1].mul(ym() / 32).sub(2)).mul(m0).mul(m1).mul(m2).mul(m3)
}
...
function buy_y2ng_max(n) {
  if (y2ng.y.gte(cost(n))) {
    ...
    y2ng.glevel[n - 1] = y2ng.y.log().mul(E.LOG2E).div(E(2).pow(n)).add(0.5).floor()
...
<button name="max" type="button" class="max" onmousedown="buy_y2ng_max_all()">
  <math>
    <mo style="font-family: 'YYReset';">所有层级全部购买最大</mo>
    <mo>(</mo><mo style="font-family: 'YYReset';">快捷键</mo><mo>:</mo>
    <mo>Q</mo><mo>)</mo>
  </math>
</button>`
...
function display_2n(n) {
  return n == 0 ? `<mi>y</mi>` : `<msup>
  <mi>y</mi>
  <mn>${2 ** n}</mn>
</msup>`
}

function display_autobuy(n) {
  return `<button name="buymax" type="button" class="y${2 ** n}" 
    onmousedown="y2ng.autobuy[${n - 1}] = !y2ng.autobuy[${n - 1}]">
  <math>
    <mo style="font-family: 'YYReset';">自动购买最大</mo><mo>:</mo>
    <mo style="font-family: 'YYReset';">${y2ng.autobuy[n - 1] ? "是" : "否"}</mo>
  </math>
</button>`
}
...
function update_y2n(a = true, b = true) {
  if (b) {
    ...
    if (y2ng.y.gt(2 ** 256) && !(achievement[13])) {
      y2ng.y = E(2 ** 256)
    }
    for (let n = 1; n < 8; n++) {
