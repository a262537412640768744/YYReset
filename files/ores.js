...
function copper_goal() {
  return E(ores.upg.iron[1] ? 2400 : 57600)
}

function display_ores() {
  return `${display_an_ore("iron", "铁", E(600), 3 / 4)}
${display_an_ore("copper", "铜", E(copper_goal()), 1 / 2)}
...
${yklng[0].y.gte(600) ? display_ore_upgrade("iron", "铁", E(100), [0, 
  ...
  display_ore_upgrade("iron", "铁", E(1600), [1, 
    `<mo style="font-family: 'YYReset';">铜矿要求</mo><mo>÷</mo><mn>24</mn>`]): ""}
<br>
${yklng[0].y.gte(copper_goal()) ? display_ore_upgrade("copper", "铜", E(200), [0, 
    display_pl(1) + `<mo style="font-family: 'YYReset';">效果增强</mo>`]) + 
  display_ore_upgrade("copper", "铜", E(3200), [1, 
    `<mo style="font-family: 'YYReset';">解锁矿石斗兽</mo>`]): ""}`
}
...
function update_ores(a = true, b = true) {
  if (b) {
    ...
    if (yklng[0].y.gte(copper_goal()) && ores.is.copper) ores.copper = ores.copper.add(
      yklng[0].y.div(copper_goal()).pow(1 / 2).mul(0.02))
  }
  if (!a) return;
