...
function format(num, precision=2, small=false) {
  ...
  if (num.lt("0.0001")) { return "<msup><mrow><mo>(</mo>" + 
    format(num.rec(), precision) + "<mo>)</mo></mrow><mn>-1</mn></msup>" }
...
function getcolor() {
  return `style="color: hsl(
  ${(new Date().getSeconds() % 12 + new Date().getMilliseconds() / 1000) * 30}, 
100%, 50%)"`
}

function display_capped() {
  return y2ng.y.gte(2 ** 256) && !(achievement[13]) ? 
    `<mo style="font-family: 'YYReset';">，已达到硬上限</mo>` : ""
}
...
${y2ng.y.gte(Endgame) ? `<math display="block" class="endgame" ` + getcolor() + `>
...
function update() {
  ...
  update_yklng(currentl, choose[0] == 1 && choose[1] == 0, !_stop)
  update_ykupg(choose[0] == 1 && choose[1] == 1, !_stop)
  update_ores(choose[0] == 1 && choose[1] == 2, !_stop)
  update_save(choose[0] == 8 && choose[1] == 0, !stop)
  update_about(choose[0] == 8 && choose[1] == 1)
  update_achievement(choose[0] == 8 && choose[1] == 2, !_stop)
  update_recommendation(choose[0] == 8 && choose[1] == 3, !_stop)
