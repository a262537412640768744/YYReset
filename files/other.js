...
</button>` + (y2ng.maxy.gte(2 ** 128) ? `
<button name="yb" type="button" class="choose" onmousedown="choose[1] = 2">
  <math display="block">
    <mi>y</mi>
...
<table>
  <tbody>
    <tr>
      ...
      <td onmouseover="m = [0, 8]" ${achievement[8] ? `style="background: #0000ff"` : ""}>
        <math display="block"><mo style="font-family: 'YYReset';">八个全有了</mo></math>
      </td>
      <td onmouseover="m = [0, 9]" ${achievement[9] ? `style="background: #0000ff"` : ""}>
...
function update_achievement(a = true, b = true) {
  if (b) {
    ...
    if (y2ng.gen[7].gte(0.5) && (achievement[8] == false)) achievement[8] = time;
