...
function get_ykl(l) {
  let y = l == 1 ? y2ng.y : yklng[l - 2].y
  return y.gte(E(2).pow(256 * l)) ? 
    y.div(E(2).pow(256 * l)).root(1024).mul(2).floor() : E(0)
}

function reset_ykl(l) {
  if (get_ykl(l).eq(0)) {
    return
  }
  yklng[l - 1].y = yklng[l - 1].y.add(get_ykl(l))
  y2ng = {
    y: E(2),
    glevel: [E(0), E(0), E(0), E(0), E(0), E(0), E(0), E(0)],
    gen: [E(0), E(0), E(0), E(0), E(0), E(0), E(0), E(0)],
...
function multiplyerl(l, n) {
  let m0 = ykupg[1] && l == 1 && n == 1 ? E(8) : E(1)
  let m1 = ores.upg.iron[0] && l == 1 && n == 1 ? E(256) : E(1)
  return E(2).pow(yklng[l - 1].glevel[n - 1].mul(ym() / 32).sub(2)).mul(m0).mul(m1)
}

function speedl(l, n) {
  return yklng[l - 1].gen[n - 1].mul(multiplyerl(l, n))
}
...
function buy_ykln_max(l, n) {
  if (yklng[l - 1].y.gte(costl(l, n))) {
    ...
    yklng[l - 1].glevel[n - 1] = yklng[l - 1].y.log().mul(E.LOG2E).div(E(2).pow(n)).add(0.5).floor()
...
function display_reset(l) {
  return `<button onmousedown="reset_ykl(${l})">
  <math display="block" class="ykln">
    <mo style="font-family: 'YYReset';">重置获得</mo>
    ${format(get_ykl(l))}
    ${display_l2n(l, 0)}
    <mo style="font-family: 'YYReset';">，下一个在</mo>
    ${format((get_ykl(l)).add(1).div(2).pow(1024).mul(E(2).pow(256 * l)))}
    ${display_2n(0)}
  </math>
</button>`
}
...
function display_ykl(l) {
  ...
  ${display_pl(l)}
  <mo style="font-family: 'YYReset';">，使</mo>
  ${display_2n(1)}
  <mo style="font-family: 'YYReset';">、</mo>
  ${display_2n(2)}
  <mo style="font-family: 'YYReset';">、</mo>
  ${display_2n(4)}
  <mo style="font-family: 'YYReset';">、</mo>
  ${display_2n(8)}
  <mo style="font-family: 'YYReset';">生成器倍数</mo><mo>×</mo>
  ${format(yklng[0].p.div(ores.upg.copper[0] && l == 1 ? 1 : 8).add(1))}
</math>
${get_ykl(l).eq(0) ? "" : display_reset(l)}`
}
...
<button name="buy" type="button" class="y${2 ** n}" onmousedown="buy_ykln(${l}, ${n})">
  <math>
...
<button name="buymax" type="button" class="y${2 ** n}" onmousedown="buy_ykln_max(${l}, ${n})">
  <math>
